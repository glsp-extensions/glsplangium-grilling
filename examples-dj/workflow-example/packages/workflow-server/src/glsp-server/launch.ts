 ...
 */
export function startGLSPServer(
  services: WorkflowLSPServices,
  ...
  const logger =
    appContainer.get<LoggerFactory>(LoggerFactory)("WorkflowServer");
  const launcher =
    appContainer.resolve<SocketServerLauncher>(SocketServerLauncher);
  ...
  const serverModule = new ServerModule().configureDiagramModule(
 ...
 */
export function createLSPModule(
  services: WorkflowLSPServices
): ContainerModule {
  return new ContainerModule((bind) => {
    bind(WorkflowLSPServices).toConstantValue(services);
    bind(WorkflowDiagramSharedServices).toConstantValue(services.shared);
    bind(WorkflowDiagramServices).toConstantValue(services.language);
  });
}
