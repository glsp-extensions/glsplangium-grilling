...
export class WorkflowDiagramJsonSerializer implements JsonSerializer {
  ...
  protected getRefNode<T extends AstNode>(
    ...
    if (ref.__id) {
      if (ref.__documentUri) {
        ...
        return this.getAstNodeById(
          doc.parseResult.value,
          ref["__id"] as string
        )!;
      }
      return this.getAstNodeById(root, ref["__id"] as string)!;
    } else if (ref.__path) {
      if (ref.__documentUri) {
        const doc = this.langiumDocs.getOrCreateDocument(
          URI.parse(ref.__documentUri)
