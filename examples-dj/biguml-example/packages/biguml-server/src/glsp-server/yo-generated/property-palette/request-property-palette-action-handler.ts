...
import { TestPropertyPaletteHandler } from './elements/TestPropertyPaletteHandler.js';
...
export class RequestPropertyPaletteActionHandler implements ActionHandler {
    ...
    execute(action: RequestPropertyPaletteAction): MaybePromise<any[]> {
        ...
        const dataTypeChoices = this.modelState.index.getAllDataTypes().map(item => ({
            label: item.name,
            value: item.__id + '_refValue',
            secondaryText: item.$type
        }));
        const definingFeatureChoices = this.modelState.index.getAllDefiningFeatures().map(item => ({
            label: item.name,
            value: item.__id + '_refValue',
            secondaryText: item.$type
        }));
        ...
        } else if (isTest(semanticElement)) {
            return TestPropertyPaletteHandler.getPropertyPalette(semanticElement);
