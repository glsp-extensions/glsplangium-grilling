import { LangiumDeclaration } from "@borkdominik/model-management-common";
import fs from "fs";
...
export function writePropertyPaletteHandlers(
  ...
  const requestFiles = writeRequestPropertyPaletteHandlers(
    extensionPath,
    declarations
  );
...
function emitBuilderLine(
  ...
  const dyn = prop.decorators?.find((d: string) =>
    d.startsWith("dynamicProperty:")
  );
  if (dyn) {
