...
import { transformEcoreToTsDefinition } from "./ecore-transformer";
import { EcoreClass, EcoreDefinition, EcoreType } from "./ecore-types";
...
export async function parseEcoreDefinitionFile(_path: any): Promise<any> {
  ...
  let formattedDefinitionFile = await prettier.format(tsDefinition, {
    parser: "typescript",
    trailingComma: "es5",
  });
  fs.writeFileSync(
