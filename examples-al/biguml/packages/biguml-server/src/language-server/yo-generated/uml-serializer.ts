...
export class UmlSerializer implements Serializer<Diagram>, DiagramSerializer<Diagram> {
    ...
    serializeAstNode(element: AstNode): string {
        if (isDiagram(element)) {
            return this.serialize(element);
        }
        if (isUnionType_0(element)) {
            return this.serializeUnionType_0(element);
        }
        if (isMetaInfo(element)) {
            return this.serializeMetaInfo(element);
        }
    ...
    serializeRelation(element: Relation): string {
        ...
        if (isInheritance(element)) {
            return this.serializeInheritance(element);
        }
    ...
    serializeInheritance(element: Inheritance): string {
        let str: Array<string> = [];
        str.push('"__type": "Inheritance"');
        if (element.__id !== undefined && element.__id !== null) {
            str.push('"__id": ' + '"' + element.__id + '"');
        }
