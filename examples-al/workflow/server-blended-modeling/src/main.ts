...
import { createWorkflowServices } from "./language-server/workflow-module.js";
...
const { shared, Workflow } = createWorkflowServices({ connection, ...NodeFileSystem });

// Start the language server with the shared services
startLanguageServer(shared);

// Start the graphical language server with the shared services
// startGLSPServer({ shared, language: Workflow });
// Start the JSON server with the shared services
// startModelServer({ shared, language: Workflow });

shared.workspace.WorkspaceManager.onWorkspaceInitialized((workspaceFolders) => {
  // Start the graphical language server with the shared services
  startGLSPServer({ shared, language: Workflow });
  // Start the JSON server with the shared services
  startModelServer({ shared, language: Workflow }, workspaceFolders[0]);
});
